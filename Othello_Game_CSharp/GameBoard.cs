...
namespace Othello_Game_CSharp
{
    public class GameBoard
    {
        ...
        private bool canFlip(Player i_Player, int i_Row, int i_Col, int i_DirectionRow, int i_DirectionCol)
        {
            ...
            int checkRow = i_Row + i_DirectionRow;
            int checkCol = i_Col + i_DirectionCol;

            while (checkRow >= 0 && checkRow < this.m_Board.GetLength(0) && checkCol >= 0 &&
                checkCol < this.m_Board.GetLength(0) && (this.m_Board[checkRow, checkCol] == i_Player.GetOpponentState()))
            {
                checkRow += i_DirectionRow;
                checkCol += i_DirectionCol;
            }

            if (checkRow < 0 || checkRow > this.m_Board.GetLength(0) - 1 || checkCol < 0 ||
                checkCol > this.m_Board.GetLength(0) - 1 || (checkRow - i_DirectionRow == i_Row && checkCol - i_DirectionCol == i_Col) ||
                this.m_Board[checkRow, checkCol] != i_Player.State)
            {
                isPossible = false;
            }
